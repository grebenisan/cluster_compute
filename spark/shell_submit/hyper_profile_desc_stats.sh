...
#	hadoop fs -put -f tbl_list5.txt    "/DEV/..../OPS/CHEV/HYPER_PROF_RSLT/Input";
#	or:
#	hadoop fs -put -f dc_list5.txt     "/DEV/..../OPS/CHEV/HYPER_PROF_RSLT/Input";
#
...
{
	local howmany=$1
	Toronto="$(TZ=US/Central date +"%Y/%m/%d %H:%M:%S")";
	...
	case $howmany in
		0) printf "[Toronto=${Toronto}]";;
		1) printf "[Toronto=${Toronto}]";;
		2) printf "[Toronto=${Toronto}] [Local=${Eastern}]";;
		*) printf "[Toronto=${Toronto}] [Local=${Eastern}] [GMT=${GMT}]";;
	esac;
}
...
Options:	-h = Help
		...
		-H = HDFS RootDir 		(Default: /\$ENV/EDW/....../CHEV/HYPER_PROF_RSLT/)
		-n = DeployMode			(Default: CLUSTER;        2 valid values:  Cluster or Client)
		-L = Root Log Dir		(Default: CHEV_STAT;  Based on Project)
		-p = Python Program		(Default: hyper_profile_desc_stats.py;)
		-P = ProjectName		(Default: CHEV_STAT;  Used to build OS LogDir)
		...
		-Y = Yarn SparkName		(Defualt: CHEV_STAT;  Show up in YARN page, for easy query)
	\n\n\n";
...
--    (NOTE:  This is assuming it has got onto to the RUNNING queue for the MicroService to populate these 2 tables (QUEUE/OUTPUT))
SELECT	PRTY_NBR, table_id, batch_id, task_stat_cd, fail_cnt, crt_by, crt_ts, upd_By, upd_ts, srvr_nm
FROM	CHEV.PROF_COL_CLS_QUE
WHERE	batch_id = $l_BATCH_ID
AND	task_stat_cd = 1 ;


-- 2) Reset BATCH_ID for these above tables:
UPDATE  CHEV.PROF_COL_CLS_QUE
SET	TASK_STAT_CD = 0,
	BATCH_ID     = NULL
WHERE	
	batch_id     = $l_BATCH_ID
...
function SendMail
{
	...
	do
		...
		cat $l_MailFile | mailx -s "CHEV BASIC STAT ($ENV): $l_JobStatus"  $em
	done
}
...
#### Ex: output_tablesWithSIZE=/data/commonScripts/util/CHEV/STAT/MDB/tbl_list3000.txt/LOGS/20180703_200828/CP_TablesSize_Try1.txt
...
#---------------------------------------------------------------------------------------------------
# STEP 1: Sample OUTPUT File: $l_Schema_TO_HDFS_Dirs: /data/commonScripts/util/CHEV/STAT/DC/dc_list800.txt/LOGS/20180910_214429/Schema_TO_HDFS_Dirs.txt
#	DL_EDGE_BASE_BIS_13426_BASE_OBISDGXP_BIS.BIS_ORDER_LINE hdfs://edwbidevwar/DEV/DL/EDGE_BASE/BIS-13426/OBISDGXP-BIS/BIS_ORDER_LINE/Data/
#---------------------------------------------------------------------------------------------------
...
function func_invoke_PySpark  
{
		...
		local l_toRunDir="$2";
		local l_splitFile=$3;	    
		# Ex: /data/commonScripts/util/CHEV/STAT/MDB/tbl_list100.txt/TORUN/20180510_095532__100__1__tbl_list100.txt
		#                                            ^          ^                    ^
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	printf "${itab3}From:\t$l_toRunDir\n${itab3}To:\t($archiveLINUXDir)\n"        >> $l_INVOKE_SPARK_LOG
		# Ex: Move OS Split_File:   /data/commonScripts/util/CHEV/STAT/MDB/tbl_list1.txt/TORUN/20180709_192508__j1__e10__t1__tbl_list1.txt
		#     to archive_LINUX_Dir: /data/commonScripts/util/CHEV/STAT/MDB/tbl_list1.txt/ARCHIVE
...
export G_MAIN_PROG_DIR="/data/commonScripts/util";			# 
		...
		#DEBUG  DEBUG DEBUG
		#export G_MAIN_PROG_DIR="/home/tqzpftv/CHEVL/SINGLETHREAD2"
		#export G_MAIN_PROG_DIR="/home/tqzpftv/CHEVL/PARALLEL"
...
export G_HDFS_ROOT_DIR="/EDW/....../CHEV/HYPER_PROF_RSLT";	# Default; Override with -H;  No specify $ENV
...
export G_ROOT_PROGDIR="/data/commonScripts/util/CHEV/STAT";	# Default; Override with -L
...
export G_YARN_SPARK_NAME="CHEV_STAT";				# Default; Override with -N; To build YARN SPARK NAME
export G_MUST_HAVE_HIVE_DATATYPES="YES";			# Default; Override with -o

export G_PROJECT_DIRNAME="CHEV_STAT";			# Default; Override with -P  (Linux Root Dir for this input file): 
...
do
        ...
        h)      Usage; exit;;						#
        H)                           p_HDFS_ROOT_DIR="$OPTARG";;	# Default:   /$ENV/EDW/....../CHEV/HYPER_PROF_RSLT/
        ...
        L)                            G_ROOT_PROGDIR="$OPTARG";;	# Linux LogDir (for logging)	    Default="/data/commonScripts/util/CHEV/STAT";
        ...
        p)                                G_PRIORITY="$OPTARG";;	# Used as Directory Name;	Default="CHEV_STAT" for G_PROJECT_TYPE=BSTAT
        P)                         p_PROJECT_DIRNAME="$OPTARG";;	# Used as Directory Name;	Default="CHEV_STAT" for G_PROJECT_TYPE=BSTAT
        r)                             G_RESET_NORUN="YES";;		# 
...
#       R)                                   G_RESET="YES";;		# Clear out HDFS Checkpoint Dir;	Deafult=NO
        ...
        Y)                         p_YARN_SPARK_NAME="$OPTARG";;	# YARN JobName;		Default=CHEV_STAT
        esac;
done;
shift $((OPTIND - 1));
	...
	"BSTAT")	
		case "$ENV" in
			"DEV")	
				...
				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway.cp-epg2i.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro-dev.cp-epg2i.domain.com/basicstats/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro-dev.cp-epg2i.domain.com/basicstats/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro-dev.cp-epg2i.domain.com/basicstats/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro-dev.cp-epg2i.domain.com/basicstats/que/update";

				export           G_MS_AUTHENTICATE_URL="https://chev-messaging-dev.cp-epg2i.domain.com/authenticate";	# BSTAT to pass a message for each completed Table
				export               G_MS_OUTBOUND_URL="https://chev-messaging-dev.cp-epg2i.domain.com/events/basic_stats/outbound/updateQueue";	# BSTAT to pass a message for each completed Table
				export       G_MS_MESSAGING_SECRET_KEY="3YmCdvHBKX;UwXrn"
				;;
			"TST")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"

				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway-test.cp-epg2i.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro-test.cp-epg2i.domain.com/basicstats/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro-test.cp-epg2i.domain.com/basicstats/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro-test.cp-epg2i.domain.com/basicstats/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro-test.cp-epg2i.domain.com/basicstats/que/update";

				export           G_MS_AUTHENTICATE_URL="https://chev-messaging-test.cp-epg2i.domain.com/authenticate";	# BSTAT to pass a message for each completed Table
				export               G_MS_OUTBOUND_URL="https://chev-messaging-test.cp-epg2i.domain.com/events/basic_stats/outbound/updateQueue";	# BSTAT to pass a message for each completed Table
				export       G_MS_MESSAGING_SECRET_KEY="3YmCdvHBKX;UwXrn"
				;;
			"CRT")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"
				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway-qa.cp-epg2i.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro-qa.cp-epg2i.domain.com/basicstats/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro-qa.cp-epg2i.domain.com/basicstats/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro-qa.cp-epg2i.domain.com/basicstats/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro-qa.cp-epg2i.domain.com/basicstats/que/update";

				export           G_MS_AUTHENTICATE_URL="https://chev-messaging-qa.cp-epg2i.domain.com/authenticate";	# BSTAT to pass a message for each completed Table
				export               G_MS_OUTBOUND_URL="https://chev-messaging-qa.cp-epg2i.domain.com/events/basic_stats/outbound/updateQueue";	# BSTAT to pass a message for each completed Table
				export       G_MS_MESSAGING_SECRET_KEY="3YmCdvHBKX;UwXrn"
				;;
			"PRD")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"

				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway.cpi.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro.cpi.domain.com/basicstats/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro.cpi.domain.com/basicstats/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro.cpi.domain.com/basicstats/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro.cpi.domain.com/basicstats/que/update";

				#export           G_MS_AUTHENTICATE_URL="https://chev-messaging.cp-epg2i.domain.com/authenticate";	# BSTAT to pass a message for each completed Table
				#export               G_MS_OUTBOUND_URL="https://chev-messaging.cp-epg2i.domain.com/events/basic_stats/outbound/updateQueue";	# BSTAT to pass a message for each completed Table
				export            G_MS_AUTHENTICATE_URL="https://chev-messaging.cpi.domain.com/authenticate";									# BSTAT to pass a message for each completed Table
				export                G_MS_OUTBOUND_URL="https://chev-messaging.cpi.domain.com/events/basic_stats/outbound/updateQueue";		# BSTAT to pass a message for each completed Table
		...
		export G_YARN_SPARK_NAME="CHEV_STAT"; 
	...
	"DC")		
		case "$ENV" in
			"DEV")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"
				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway.cp-epg2i.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro-dev.cp-epg2i.domain.com/dataclass/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro-dev.cp-epg2i.domain.com/dataclass/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro-dev.cp-epg2i.domain.com/dataclass/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro-dev.cp-epg2i.domain.com/dataclass/que/update";
				;;
			"TST")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"
				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway-test.cp-epg2i.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro-test.cp-epg2i.domain.com/dataclass/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro-test.cp-epg2i.domain.com/dataclass/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro-test.cp-epg2i.domain.com/dataclass/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro-test.cp-epg2i.domain.com/dataclass/que/update";
				;;
			"CRT")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"
				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway-qa.cp-epg2i.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro-qa.cp-epg2i.domain.com/dataclass/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro-qa.cp-epg2i.domain.com/dataclass/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro-qa.cp-epg2i.domain.com/dataclass/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro-qa.cp-epg2i.domain.com/dataclass/que/update";

				;;
			"PRD")	
				export	G_MS_AUTHORIZATION_KEY="Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..-5Bkwe7Z6QSt6DuStpye0sYSgwEonBzy-UHXMCzpVcs"
				export          G_MS_GET_TABLEBYID_URL="https://edge-gateway.cpi.domain.com/roadbuilder/tables";
				export          G_MS_GET_WORK_UNIT_URL="https://chev-micro.cpi.domain.com/dataclass/que/getworkunit"
				export G_MS_WRITE_1TABLE_TO_OUTPUT_URL="https://chev-micro.cpi.domain.com/dataclass/out/save";
				export      G_MS_SAVE_BATCH_OUTPUT_URL="https://chev-micro.cpi.domain.com/dataclass/out/savebatch";
				export           G_MS_UPDATE_QUEUE_URL="https://chev-micro.cpi.domain.com/dataclass/que/update";
				;;
			*)	;;
		esac ;
