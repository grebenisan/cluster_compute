...
class HyperProfiling:
    ...
    def get_url(cls, endpoint):
        ...
        if cls._endpoint_env == 'dev':
            url = 'https://chev-micro-dev-enc.cp-epg2i.domain.com' + endpoint
            return url
            ...
            url = 'https://chev-micro.cp-epg2i.domain.com' + endpoint
            return url
                    ...
                    try:
                        ...
                        try:
                            ...
                            if failed_update_response['status'] != 200:
                                ...
                                r = requests.post(
                                    'https://chev-elk-logger.cp-epg2i.domain.com/logging/chev_dc/stdout',
                                    json=failed_log)
                                r.close()
                                continue

                        update_log = {'content': json.dumps(empty_update_schema), 'status':update_response['status']}
                        r = requests.post('https://chev-elk-logger.cp-epg2i.domain.com/logging/chev_dc/stdout',
                                          json=update_log)
                        r.close()
                        continue
