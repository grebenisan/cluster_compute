...
"""
Title: Chev Hyper Data Profiling Data Classification
Description: Find suspected columns to encrypt.
Version: 1.1
Python Version: 3.5
"""
...
class HyperProfiling:
    ...
    _spark = SparkSession.builder.appName("chev Hyper Data Profiling").enableHiveSupport().getOrCreate()
    _env = _spark._jsc.hadoopConfiguration().get('dfs.internal.nameservices')[5:-3].upper()
    _endpoint_env = _env.lower()

    def __init__(self):
        self.app_name = 'chev Hyper Data Profiling'
    ...
    @staticmethod
    def get_auth_token():
        """Request JWT for authentication to the microservice"""
        auth = {'SECRET_ID': '3YmCdvHBKX;UwXrn'}
        response = requests.post(__class__.get_url('/authenticate').get('messaging_url'), json=auth)
        access_token = json.loads(response.content.decode('utf-8'))['access_token']
        return access_token

    @staticmethod
    def update_message_queue(message, token):
        """Sends profiling status to chev outbound message queue"""
        ...
        session = __class__.get_session()
        with session as session_object:
            session_object.headers.update(header)
            response = \
                session_object.post(__class__.get_url('/events/data_class/outbound/updateQueue').
                                    get('messaging_url'),
                                    json=message)
            return response
    ...
    def get_url(cls, endpoint):
        ...
        if cls._endpoint_env == 'dev':
            url = 'https://chev-micro-dev.cp-epg2i.domain.com' + endpoint
            messaging_url = 'https://chev-messaging.cp-epg2i.domain.com' + endpoint
            return {'url': url, 'messaging_url': messaging_url}
        elif cls._endpoint_env == 'tst':
            url = 'https://chev-micro-test.cp-epg2i.domain.com' + endpoint
            messaging_url = 'https://chev-messaging-test.cp-epg2i.domain.com' + endpoint
            return {'url': url, 'messaging_url': messaging_url}
        elif cls._endpoint_env == 'crt':
            url = 'https://chev-micro-qa.cp-epg2i.domain.com' + endpoint
            messaging_url = 'https://chev-messaging-qa.cp-epg2i.domain.com' + endpoint
            return {'url': url, 'messaging_url': messaging_url}
        else:
            url = 'https://chev-micro.cpi.domain.com' + endpoint
            messaging_url = 'https://chev-messaging.cp-epg2i.domain.com' + endpoint
            return {'url': url, 'messaging_url': messaging_url}

    def classify_data(self, file_name):
        """Profiles columns from input file."""
        access_token = __class__.get_auth_token()
        try:
            session_id = datetime.datetime.now(CENTRAL_TIMEZONE).strftime("%Y-%m-%d %H:%M:%S")
            print("\n\n=====Python Job starts at {session_id}\n".format(session_id=session_id))

            save = HyperProfiling.get_url('/dataclass/out/save').get('url')
            update = HyperProfiling.get_url('/dataclass/que/update').get('url')
            ...
            hdfs_output_dir = '/{env}/EDW/......./chev/HYPER_PROF_RSLT/Output'.format(env=self._env)
            input_path = '/{env}/EDW/......./chev/HYPER_PROF_RSLT/Input/{file_name}'
            ...
            for tables in input_tables_broadcast.value:
                ...
                for row in list(rows_to_profile):
                    ...
                    empty_ts = datetime.datetime.now(CENTRAL_TIMEZONE).strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    try:
                        ...
                        try:
                            ...
                            summary_msg = '{output}\t' \
                                          'CRITICAL FAIL\t' \
                                          'Check Update Endpoint'.format(output=json.dumps(failed_update_schema))
                            print(summary_msg)
                            sys.exit(0)

                        event = failed_update_schema
                        message = json.dumps(event)
                        __class__.update_message_queue(message, access_token)
                        print('Failed', failed_update_schema, update_response['status'])
                        continue
                        ...
                        if not data_empty:
                            ...
                            try:
                                table_data_column = table_data.select(col_name). \
                                        ...
                                        where(lower(trim(table_data[col_name])).
                                              isin(' ', 'null', 'n/a', 'unknown', 'unk',
                                                   'unspecified', 'no match row id',
                                ...
                                if column_row_count_full >= 1000000:
                                    ...
                                    try:
                                        profile_start = \
                                            datetime.datetime.now(CENTRAL_TIMEZONE).strftime("%Y-%m-%d %H:%M:%S")
                                        ...
                                        update_output = UpdateQueue({'update': update, 'session': session})
                                        update_response = \
                                            update_output.save_results({'update_output': success_update_schema})
                                        payload = \
                                            json.loads(final_results.toJSON().collect()[0],
                                        ...
                                        if update_response['status'] != 200:
                                            summary_msg = '{output}\t' \
                                                          ...
                                                          format(output=json.dumps(failed_update_schema))
                                            print(summary_msg)
                                            sys.exit(0)
                                        ...
                                        failed_update_output = UpdateQueue({'update': update, 'session': session})
                                        failed_update_response = \
                                            failed_update_output.save_results({'update_output': failed_update_schema})
                                        exc_type, exc_value, exc_tb = sys.exc_info()
                                        print(traceback.format_exception(exc_type, exc_value, exc_tb))
                                        if failed_update_response['status'] != 200:
                                            ...
                                            message = json.dumps(event)
                                            __class__.update_message_queue(message, access_token)
                                            print('Failed', failed_update_schema, failed_update_response['status'])
                                            continue

                                    event = success_update_schema
                                    message = json.dumps(event)
                                    __class__.update_message_queue(message, access_token)
                                    print('Success', success_update_schema, update_response['status'])
                            ...
                            event = empty_update_schema
                            message = json.dumps(event)
                            __class__.update_message_queue(message, access_token)
                            print('Empty', empty_update_schema, update_response['status'])
                            continue
